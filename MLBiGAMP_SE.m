...
for ii=1:IterNum 
     if AGC_switch==1
        for Index=1:length(y_set)      
            alpha(Index)=integral(@(xi) (Gaussian(sqrt(2)*y_up(Index),sqrt(chi_z-V)*xi,nuw+V)-Gaussian(sqrt(2)*y_low(Index),sqrt(chi_z-V)*xi,nuw+V)).^2 ...
                      ./(normcdf((sqrt(2)*y_up(Index)-sqrt(chi_z-V)*xi)/(sqrt(nuw+V)))-normcdf((sqrt(2)*y_low(Index)-sqrt(chi_z-V)*xi)/(sqrt(nuw+V)))+eps).*Gaussian(xi,0,1),-Inf,Inf);
        end
        alpha=sum(alpha);
        mse_z=V-V^2*alpha;
        ...
        gamma_x=1/Sigma_x;
        MSE(ii)=1-integral(@(u) tanh(gamma_x+sqrt(gamma_x).*u).*Gaussian(u,0,1),-Inf,Inf);
        q_x=chi_x-MSE(ii);
        V=N*chi_h*(chi_x-q_x);
     else 
